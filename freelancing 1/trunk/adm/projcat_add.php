...
if(isset($_POST))
{
	if(count($_POST) > 1)
	{
		mysql_query("INSERT INTO `".$config['db']['pre']."categories` ( `cat_id` , `parent_id` , `cat_name` ) VALUES ('', '" . addslashes($_POST['parent']) . "', '" . addslashes($_POST['title']) . "');");
		
		transfer($config,'projcat_edit.php','Project Category Added');
		exit;
	}
}
...
<table width="850" cellpadding="0" cellspacing="0" border="0" align="center">
<tr>
<td class="heading"><img src="images/icon_addrule.gif" width="19" height="24" alt="" align="absmiddle" hspace="5">Add Project Category</td>
</tr>
<tr>
<td bgcolor="#333333"><img src="images/dot.gif" width="1" height="1" alt=""></td>
</tr>
...
<td align="center" bgcolor="#F6F6F6" style="padding:15px;">  <form name="form1" method="post" action="">
  <table width="70%" cellpadding="0" cellspacing="2" border="0">
    <tr>
      <td width="25%"><strong>Category Title</strong></td>
      <td width="55%">:
        <input name="title" type="Text" class="textbox" id="id" style="width:60%" value=""></td>
    </tr>
    <tr>
      <td width="25%"><strong>Category Parent</strong></td>
      <td width="55%">:
        <select name="parent" style="width:60%">
        <option value="0">None</option>
<?
$query = "SELECT cat_id,cat_name FROM ".$config['db']['pre']."categories WHERE parent_id='0' ORDER BY cat_name";
$query_result = mysql_query($query);
while ($info = @mysql_fetch_array($query_result))
{
	echo '<option value="'.$info['cat_id'].'">'.stripslashes($info['cat_name']).'</option>';
}
?>
        </select></td>
    </tr>
