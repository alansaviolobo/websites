...
<html>
<head>
<title><?php echo $config['site_title']; ?> Admin - View Subscribers</title>
...
<table width="850" border="0" align="center" cellpadding="0" cellspacing="0" style="border:1px solid #CCCCCC;">
  <tr>
    <td align="center" bgcolor="#F6F6F6" style="padding:15px;"><div align="right">
        <form action="subscriptions_delete.php" method="post" name="f1" id="f1">
          <div align="right"> <br>
          </div>
          <table width="99%" border="0" align="center" cellpadding="2" cellspacing="0">
            <tr bgcolor="#FF9900">
              ...
              <td height="30"><span class="rowheader">User Type</span></td>
              <td height="30"><span class="rowheader">Upgrade Name</span></td>
...
$query = "SELECT 1 FROM ".$config['db']['pre']."upgrades";
...
$query = "SELECT upgrade_id,sub_id,user_type,user_id FROM ".$config['db']['pre']."upgrades ".$limit;
...
while ($info = @mysql_fetch_array($query_result))
{
	...
	$upgrade_info = mysql_fetch_row(mysql_query("SELECT sub_title FROM ".$config['db']['pre']."subscriptions WHERE sub_id='".addslashes($info['sub_id'])."' LIMIT 1"));
...
?>
            <tr bgcolor="<?php echo $colour; ?>">
              <td width="50" height="25" align="center"><input type="hidden" name="titles[]" id="titles[]" value="<?php echo $username;?>">
                  <input type="checkbox" name="list[]" id="list[]" value="<?php echo $info['upgrade_id'];?>"></td>
              <td height="25"><span class="style5"><?php echo $username;?></span></td>
              <td><? if($info['user_type']=='buyer'){ echo $lang['BUYERU']; } else { echo $lang['PROVIDERU']; } ?></td>
              <td><? echo stripslashes($upgrade_info[0]); ?></td>
			  <td height="25">
			  <table border="0" cellpadding="0" cellspacing="0"><tr><td>
                <select name="amenu<?php echo $info['upgrade_id'];?>" id="amenu<?php echo $info['upgrade_id'];?>" onChange="MM_jumpMenu('parent',this,0)" style="width:145px">
				  <option value="">Options</option>
                  <option value="subscriptions_edit2.php?id=<?php echo $info['upgrade_id'];?>">View/Edit Subscription</option>
                  <option value="subscriptions_delete.php?id=<?php echo $info['upgrade_id'];?>">Cancel Subscription</option>
                </select>
</td>
			  </tr>
			  </table>
...
?>
          ...
          <div align="left"> <br>
              <table width="99%"  border="0" align="center" cellpadding="2" cellspacing="0">
                <tr>
                  <td width="200" valign="middle">With Selected:&nbsp;<a href="#" onClick="document.f1.action='subscriptions_edit2.php'; document.f1.submit();"><img src="images/button_edit.gif" width="12" height="13" border="0"></a>
