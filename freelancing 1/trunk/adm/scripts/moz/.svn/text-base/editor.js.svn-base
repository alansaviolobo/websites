    ...
    for(var i=0;i<this.buttonMap.length;i++)
        ...
        switch(sButtonName)
			...
			case "SpellCheck":
				if(this.btnSpellCheck)
					...
					if(this.spellCheckMode=="NetSpell")
						sHTMLIcons+=this.writeIconStandard("btnSpellCheck"+this.oName,this.oName+".hide();modalDialogShow('"+this.scriptPath+"spellcheck2.htm',500,500)","btnSpellCheck.gif",getText("Check Spelling"));
					}
				break;
			...
			case "PasteWord":
				...
				break;
			case "PasteText":
				if(this.btnPasteText)sHTMLIcons+=this.writeIconStandard("btnPasteText"+this.oName,"modelessDialogShow('"+this.scriptPath+"paste_text.htm',400,280)","btnPasteText.gif",getText("Paste Text"));
				break;
            case "Bold":
...
function ApplyExternalStyle(oName)
    {
    //if(eval(oName).isCssLoaded)return;
    ...
    if(arrStyle.length>0)
        ...
        for(var i=0;i<arrStyle.length;i++)
            ...
            sCssText += selector + " { " + style + " } ";    
            if(style!="")
                oEditor.document.styleSheets[0].insertRule(selector + " { " + style + " } ", 0);//TODO: Gunanya?
            }
        oElement.appendChild(oEditor.document.createTextNode(sCssText));        
        }
