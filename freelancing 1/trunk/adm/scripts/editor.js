/***********************************************************
InnovaStudio WYSIWYG Editor 2.7
...
function RENDER(sPreloadHTML)
	...
	for(var i=0;i<this.buttonMap.length;i++)
		...
		switch(sButtonName)
			...
			case "SpellCheck":
				if(this.btnSpellCheck)
					...
					if(this.spellCheckMode=="ieSpell")
						sHTMLIcons+=this.writeIconStandard("btnSpellCheck"+this.oName,this.oName+".hide();windowOpen('"+this.scriptPath+"spellcheck.htm',500,222)","btnSpellCheck.gif",getText("Check Spelling"));
					if(this.spellCheckMode=="NetSpell")
						sHTMLIcons+=this.writeIconStandard("btnSpellCheck"+this.oName,this.oName+".hide();windowOpen('"+this.scriptPath+"spellcheck2.htm',500,500)","btnSpellCheck.gif",getText("Check Spelling"));
					}
				break;
			case "StyleAndFormatting":
				...
				if(this.btnCssBuilder) 
					sTmp+= "<tr><td onclick=\"modelessDialogShow('"+this.scriptPath+"styles_cssText2.htm',430,445);"+
						...
						" style=\"padding:2px;padding-top:1px;font-family:Tahoma;font-size:11px;color:black;\" "+
						"onmouseover=\"this.style.backgroundColor='#708090';this.style.color='#FFFFFF';\" "+
						"onmouseout=\"this.style.backgroundColor='';this.style.color='#000000';\" unselectable=on>"+getText("CSS Builder")+"</td></tr>";
				...
				break;
			case "Styles":
				if(this.btnStyles)sHTMLIcons+=this.writeIconStandard("btnStyles"+this.oName,this.oName+".hide();"+this.oName+".openStyleSelect()","btnStyleSelect.gif",getText("Style Selection"));
				break;
			case "Paragraph":
			...
			case "Redo":
				...
				break;
			case "Bold":
				if(this.btnBold)sHTMLIcons+=this.writeIconToggle("btnBold"+this.oName,this.oName+".applyBold()","btnBold.gif",getText("Bold"));
				break;
			case "Italic":
				if(this.btnItalic)sHTMLIcons+=this.writeIconToggle("btnItalic"+this.oName,this.oName+".applyItalic()","btnItalic.gif",getText("Italic"));
				break;
			case "Underline":
				if(this.btnUnderline)sHTMLIcons+=this.writeIconToggle("btnUnderline"+this.oName,this.oName+".applyLine('underline')","btnUnderline.gif",getText("Underline"));
				break;
			case "Strikethrough":			
				if(this.btnStrikethrough)sHTMLIcons+=this.writeIconToggle("btnStrikethrough"+this.oName,this.oName+".applyLine('line-through')","btnStrikethrough.gif",getText("Strikethrough"));
				break;
			case "Superscript":
			...
			case "RTL":
				...
				break;
			case "ForeColor":
				if(this.btnForeColor)sHTMLIcons+=this.writeIconStandard("btnForeColor"+this.oName,this.oName+".oColor1.show(this)","btnForeColor.gif",getText("Foreground Color"));
				break;
			case "BackColor":
				if(this.btnBackColor)sHTMLIcons+=this.writeIconStandard("btnBackColor"+this.oName,this.oName+".oColor2.show(this)","btnBackColor.gif",getText("Background Color"));
				break;
			case "Bookmark":
			...
			case "Form":
				...
				break;
			case "RemoveFormat":
				if(this.btnRemoveFormat)sHTMLIcons+=this.writeIconStandard("btnRemoveFormat"+this.oName,this.oName+".doClean()","btnRemoveFormat.gif",getText("Remove Formatting"));
				break;
			case "HTMLFullSource":
	...
	if(this.css!="")
		{
		//alert(eval("idContent"+this.oName).document.body.innerHTML) => ternyata eval("idContent"+this.oName) masih belum siap
		...
		eval("myStyle"+this.oName).document.write("<html><head><link href=\""+this.css+"\" rel=\"stylesheet\" type=\"text/css\"></head><body></body></html>");
...
function putHTML(sHTML)//used by source editor
	...
	/*** Only parse embedded stylesheet (not linked css) ***/
	var sTmp="";
	...
	//Combine with existing arrStyle
	for(var i=0;i<arrStyle.length;i++)
		this.arrStyle.push(arrStyle[i]);
...
function ApplyExternalStyle(oName)
	...
	if(myStyle.document.styleSheets.length==0)return;
	...
	//Combine with existing arrStyle
	for(var i=0;i<arrStyle.length;i++)
		eval(oName).arrStyle.push(arrStyle[i]);
...
function applyItalic()
	...
	else oElement.style.fontStyle="bold";
