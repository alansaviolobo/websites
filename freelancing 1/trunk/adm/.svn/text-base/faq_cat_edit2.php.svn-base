...
if(isset($_POST))
{
	if(count($_POST) > 1)
	{
		if(isset($_POST['Submit']))
		{
			...
			{
				mysql_query("UPDATE `".$config['db']['pre']."faq_cats` SET `cat_title` = '" . addslashes($_POST['title'][$value]) . "' WHERE `cat_id` = '" . $value . "' LIMIT 1 ;");
			}
			
			transfer($config,'faq_cat_edit.php','FAQ Categories Edited');
			exit;
		}
	}
}
...
<html>
<head>
	<title><?php echo $config['site_title']; ?> Admin - Edit FAQ Categories</title>
...
<?
$count = 0;
$sql = "SELECT cat_id,cat_title FROM ".$config['db']['pre']."faq_cats ";
