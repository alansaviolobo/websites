...
if(isset($_POST))
{
	if(count($_POST) > 1)
	{
		if(isset($_POST['Submit']))
		{
			...
			{
				mysql_query("UPDATE `".$config['db']['pre']."bids` SET `big_amount` = '" . addslashes($_POST['amount'][$value]) . "' WHERE `bid_id` = '" . addslashes($value) . "' LIMIT 1 ;");
			}
			
			transfer($config,'bids_view.php','Bids Edited');
			exit;
		}
	}
}
...
<table width="850" cellpadding="0" cellspacing="0" border="0" align="center">
<tr>
<td class="heading"><img src="images/icon_editrule.gif" width="19" height="24" alt="" align="absmiddle" hspace="5">Edit Bids</td>
</tr>
<tr>
<td bgcolor="#333333"><img src="images/dot.gif" width="1" height="1" alt=""></td>
</tr>
...
<?
$count = 0;
$sql = "SELECT bid_id,bid_amount FROM ".$config['db']['pre']."bids ";

foreach ($_POST['list'] as $value) 
{
	if($count == 0)
	{
		$sql.= "WHERE bid_id='" . $value . "'";
	}
	ELSE
	{
		$sql.= " OR bid_id='" . $value . "'";
...
<table width="70%" cellpadding="0" cellspacing="2" border="0">
  <tr>
    <td width="35%"><strong>Bid ID</strong></td>
    <td>:
        <input name="id[<?php echo $info['bid_id'];?>]" type="Text" class="textbox" style="width:316px" value="<?php echo $info['bid_id'];?>" disabled></td>
        <input name="id[<?php echo $info['bid_id'];?>]" type="hidden" class="textbox" style="width:316px" value="<?php echo $info['bid_id'];?>"></td>
  </tr>
  <tr>
    <td width="35%"><strong>Bid Amount</strong></td>
    <td>:
        <input name="amount[<?php echo $info['bid_id'];?>]" type="Text" class="textbox" style="width:316px" value="<?php echo stripslashes($info['bid_amount']);?>"></td>
  </tr>
</table>
<br><br>
<?
