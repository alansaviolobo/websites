...
class Menu 
{
	...
	function get_children($id)
	{
		...
		$query = "SELECT menu_id,menu_title,menu_url,menu_desc,menu_icon,menu_target FROM amenu WHERE parent_id = '$id' ORDER BY sort_id";
	...
	function get_js_menu($id, $count = 0) {
		...
		for ($x=0; $x<sizeof($children); $x++) {
			...
			if($this->get_type($children[$x]['id']) == 1) {
				$link .= $appended."['".$children[$x]['icon']."','".$children[$x]['title']."','".$children[$x]['url']."','".$children[$x]['target']."','".$children[$x]['desc']."'".", <br>\n";
			} else {
				$link .= $appended."['".$children[$x]['icon']."','".$children[$x]['title']."','".$children[$x]['url']."','".$children[$x]['target']."','".$children[$x]['desc']."'],"."<br>"."\n";
			}
