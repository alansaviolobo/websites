...
{
	echo 'Before you run an upgrade it is recomended that you backup your '.$config['site_title'].' database<br><Br>Are you sure you want to upgrade your '.$config['site_title'].' installation from '.$config['version'].' to '.$install_version.'?<br><br><a href="upgrade_1.3.php?install=1">Yes do it</a>';
}
else
{
	ignore_user_abort(1);
	...
	@mysql_query("ALTER TABLE `".$config['db']['pre']."providers` ADD `provider_paypal` VARCHAR( 255 ) NOT NULL ,ADD `provider_confirm` MEDIUMINT( 8 ) UNSIGNED NOT NULL ;") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";
	
	//	Check that the tables exist
	echo "Updating Buyer Table...  \t\t";
	@mysql_query("ALTER TABLE `".$config['db']['pre']."buyers` ADD `buyer_confirm` MEDIUMINT( 8 ) UNSIGNED NOT NULL ;") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";
	
	//	Check that the tables exist
	echo "Updating Project Table...  \t\t";
	@mysql_query("ALTER TABLE `".$config['db']['pre']."projects` ADD `project_paid` tinyint(1) unsigned NOT NULL default '0';") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";
