...
{
	echo 'Before you run an upgrade it is recomended that you backup your '.$config['site_title'].' database<br><Br>Are you sure you want to upgrade your '.$config['site_title'].' installation from '.$config['version'].' to '.$install_version.'?<br><br><a href="upgrade_1.2.php?install=1">Yes do it</a>';
}
else
{
	ignore_user_abort(1);
	...
	echo "Updating Buyer Balanace Table...  \t\t";
	@mysql_query("ALTER TABLE `".$config['db']['pre']."buyers_balance` CHANGE `balance_amount` `balance_amount` FLOAT( 8, 2 ) NOT NULL DEFAULT '0'") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";
	
	echo "Updating Providers Balanace Table...  \t\t";
	@mysql_query("ALTER TABLE `".$config['db']['pre']."providers_balance` CHANGE `balance_amount` `balance_amount` FLOAT( 8, 2 ) NOT NULL DEFAULT '0'") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";
	
	//	Check that the tables exist
	echo "Updating Provider Table...  \t\t";
	$table_info = @mysql_query("ALTER TABLE `".$config['db']['pre']."providers` ADD `provider_paypal` VARCHAR( 255 ) NOT NULL ,ADD `provider_confirm` MEDIUMINT( 8 ) UNSIGNED NOT NULL ;") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";
	
	//	Check that the tables exist
	echo "Updating Buyer Table...  \t\t";
	$table_info = @mysql_query("ALTER TABLE `".$config['db']['pre']."buyers` ADD `buyer_confirm` MEDIUMINT( 8 ) UNSIGNED NOT NULL ;") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";
	
	//	Check that the tables exist
	echo "Updating Project Table...  \t\t";
	$table_info = @mysql_query("ALTER TABLE `".$config['db']['pre']."projects` ADD `project_paid` tinyint(1) unsigned NOT NULL default '0';") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";
	...
	echo "Create Log Table... \t\t";
	@mysql_query("CREATE TABLE `".$config['db']['pre']."logs` (  `log_id` int(11) unsigned NOT NULL auto_increment,  `log_date` int(11) unsigned NOT NULL default '0',  `log_summary` varchar(100) NOT NULL default '',  `log_details` mediumtext NOT NULL,  PRIMARY KEY  (`log_id`)) ENGINE=MyISAM AUTO_INCREMENT=7 ;") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";
	
	// Add ban table
	echo "Creating the Ban Table...  \t\t";
	@mysql_query("CREATE TABLE `".$config['db']['pre']."bans` (  `ban_id` int(11) unsigned NOT NULL auto_increment,  `ban_type` varchar(20) NOT NULL default '',  `ban_value` varchar(255) NOT NULL default '',  `ban_time` int(11) unsigned NOT NULL default '0',  PRIMARY KEY  (`ban_id`)) ENGINE=MyISAM AUTO_INCREMENT=1 ;") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";
					
	// Add inbox table
	echo "Creating the Inbox Table...  \t\t";
	@mysql_query("CREATE TABLE `".$config['db']['pre']."inbox` (  `message_id` int(11) unsigned NOT NULL auto_increment,  `message_from` int(11) unsigned NOT NULL default '0',  `message_fromtype` enum('0','1') NOT NULL default '0',  `message_to` int(11) unsigned NOT NULL default '0',  `message_totype` enum('0','1') NOT NULL default '0',  `message_date` int(11) unsigned NOT NULL default '0',  `message_subject` varchar(50) NOT NULL default '',  `message_body` mediumtext NOT NULL,  `message_read` enum('0','1') NOT NULL default '0',  PRIMARY KEY  (`message_id`),  KEY `message_from` (`message_from`,`message_fromtype`),  KEY `message_to` (`message_to`,`message_totype`)) ENGINE=MyISAM AUTO_INCREMENT=1 ;") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";
	
	echo "Adding New Payment Plugins...  \t\t";
	@mysql_query("INSERT INTO `".$config['db']['pre']."payments` (`payment_deposit` ,`payment_withdraw` ,`payment_title` ,`payment_folder` ,`payment_cost` ,`payment_cost_withdraw` ,`payment_desc_deposit` ,`payment_desc_withdraw` ,`payment_settings` ,`payment_minimum_withdraw` ,`payment_minimum_deposit`) VALUES ('0', '0', 'NoChex', 'nochex', '3% + $0.30', 'No Fee', 'Make an instant deposit using online payment service <a href=\"http://www.nochex.com\" target=\"_new\">NoChex.com</a>.', 'Make a withdrawal using online payment service <a href=\"http://www.nochex.com\" target=\"_new\">NoChex.com</a>.', 'a:6:{s:12:\"deposit_cost\";s:4:\"0.30\";s:18:\"deposit_percentage\";s:1:\"3\";s:13:\"withdraw_cost\";s:1:\"0\";s:19:\"withdraw_percentage\";s:1:\"0\";s:13:\"withdraw_link\";s:165:\"echo \'<a target=\"_new\" href=\"https://secure.nochex.com/?merchant_id=\' . urlencode(\$tran_settings[\'address\']) . \'&amount=\' . \$tran_amount . \'\">Click here to pay</a>\';\";s:14:\"nochex_address\";s:18:\"sales@site.com\";}', '5', '5');");
	echo "success<br>";
	
	echo "Updating Buyers Table...  \t\t";
	mysql_query("ALTER TABLE `".$config['db']['pre']."buyers` ADD `buyer_forgot` VARCHAR( 40 ) NOT NULL ;");
	echo "success<br>";
	
	echo "Updating Providers Table...  \t\t";
	mysql_query("ALTER TABLE `".$config['db']['pre']."providers` ADD `provider_forgot` VARCHAR( 40 ) NOT NULL ;");
	echo "success<br>";

	// Add the language field
	echo "Adding language field...  \t\t";
	@mysql_query("ALTER TABLE `".$config['db']['pre']."providers` ADD `provider_lang` VARCHAR( 50 ) NOT NULL;") or install_error('ERROR ('.mysql_error().')');
	@mysql_query("ALTER TABLE `".$config['db']['pre']."buyers` ADD `buyer_lang` VARCHAR( 50 ) NOT NULL;") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";

	echo "Updating Jobs Table...  \t\t";
	@mysql_query("ALTER TABLE `".$config['db']['pre']."jobs` ADD `job_contact` MEDIUMTEXT NOT NULL ;") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";
	
	echo "Updating Providers Table...  \t\t";
	@mysql_query("ALTER TABLE `".$config['db']['pre']."providers` ADD `group_id` INT( 11 ) UNSIGNED NOT NULL DEFAULT '1' AFTER `provider_id` ;") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";
	
	echo "Updating Buyers Table...  \t\t";
	@mysql_query("ALTER TABLE `".$config['db']['pre']."buyers` ADD `group_id` INT( 11 ) UNSIGNED NOT NULL DEFAULT '1' AFTER `buyer_id` ;") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";

	echo "Create Usergroups Table...  \t\t";
	@mysql_query("CREATE TABLE `".$config['db']['pre']."usergroups` (  `group_id` int(11) unsigned NOT NULL auto_increment,  `group_removable` tinyint(1) unsigned NOT NULL default '0',  `group_name` varchar(50) NOT NULL default '',  `group_create_project` tinyint(1) NOT NULL default '1',  `group_create_job` tinyint(1) NOT NULL default '1',  `group_bid` tinyint(1) NOT NULL default '1',  PRIMARY KEY  (`group_id`))") or install_error('ERROR ('.mysql_error().')');
	mysql_query("INSERT INTO `".$config['db']['pre']."usergroups` VALUES(1, 0, 'Registered User', 1, 1, 1);") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";
	
	echo "Create Custom Fields... \t\t";
	@mysql_query("ALTER TABLE `".$config['db']['pre']."projects` ADD `project_custom_fields` MEDIUMTEXT NOT NULL ;") or install_error('ERROR ('.mysql_error().')');
	@mysql_query("ALTER TABLE `".$config['db']['pre']."projects` ADD `project_custom_values` MEDIUMTEXT NOT NULL ;") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";

	echo "Create Custom Table... \t\t";
	@mysql_query("CREATE TABLE `".$config['db']['pre']."custom_fields` (  `custom_id` int(11) unsigned NOT NULL auto_increment,  `custom_page` varchar(60) NOT NULL default '',  `custom_name` varchar(40) NOT NULL default '',  `custom_title` varchar(100) NOT NULL default '',  `custom_type` varchar(40) NOT NULL default '',  `custom_content` varchar(20) NOT NULL default '',  `custom_min` int(11) unsigned NOT NULL default '0',  `custom_max` int(11) unsigned NOT NULL default '0',  `custom_required` tinyint(1) unsigned NOT NULL default '0',  `custom_options` mediumtext NOT NULL,  `custom_default` varchar(200) NOT NULL default '',  PRIMARY KEY  (`custom_id`),  KEY `custom_page` (`custom_page`));") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";
	
	echo "Add Category Parent Field... \t\t";
	@mysql_query("ALTER TABLE `".$config['db']['pre']."categories` ADD `parent_id` MEDIUMINT( 8 ) UNSIGNED NOT NULL AFTER `cat_id`;") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";
	
	echo "Add Escrow Table... \t\t";
	@mysql_query("CREATE TABLE `".$config['db']['pre']."escrow` ( `escrow_id` int(11) unsigned NOT NULL auto_increment,  `escrow_from` char(3) NOT NULL default '',  `escrow_status` tinyint(1) unsigned NOT NULL default '0',  `escrow_amount` decimal(8,2) NOT NULL default '0.00',  `escrow_desc` varchar(200) NOT NULL default '',  `buyer_id` int(11) unsigned NOT NULL default '0',  `provider_id` int(11) unsigned NOT NULL default '0',  PRIMARY KEY  (`escrow_id`))") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";
	
	echo "Add Settings Table... \t\t";
	@mysql_query("CREATE TABLE `".$config['db']['pre']."settings` (  `setting_id` int(11) unsigned NOT NULL auto_increment,  `setting_file` varchar(100) NOT NULL default '',  `setting_title` varchar(200) NOT NULL default '',  `setting_name` varchar(100) NOT NULL default '',  `setting_type` varchar(30) NOT NULL default 'textfield',  `setting_options` mediumtext NOT NULL,  `setting_value` mediumtext NOT NULL,  `setting_display` tinyint(1) unsigned NOT NULL default '0',  PRIMARY KEY  (`setting_id`),  KEY `setting_name` (`setting_name`),  KEY `setting_file` (`setting_file`))") or install_error('ERROR ('.mysql_error().')');
	@mysql_query("INSERT INTO `".$config['db']['pre']."settings` VALUES(1, '', 'Cron Time', 'cron_time', 'hidden', '', '0', 0);") or install_error('ERROR ('.mysql_error().')');
	echo "success<br>";
	...
	$content.= "\$config['pay_type'] = '0';\n";
	$content.= "\$config['security'] = '1';\n";
	...
	$content.= "\$config['provider_com'] = '0';\n";
	$content.= "\$config['buyer_com'] = '0';\n";
	$content.= "\$config['provider_fee'] = '0';\n";
	$content.= "\$config['buyer_fee'] = '0';\n";
