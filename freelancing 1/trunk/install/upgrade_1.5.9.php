...
{
	echo 'Before you run an upgrade it is recomended that you backup your '.$config['site_title'].' database<br><Br>Are you sure you want to upgrade your '.$config['site_title'].' installation from '.$config['version'].' to '.$install_version.'?<br><br><a href="upgrade_1.5.9.php?install=1">Yes do it</a>';
}
else
{
	ignore_user_abort(1);
	...
	$pro_group_check =  mysql_num_rows(mysql_query("SHOW COLUMNS FROM ".$config['db']['pre']."providers"));
	
	if($pro_group_check == 21)
	{
	...
	{
		echo "Provider table already has ".$pro_group_check." fields<br>";
	}
	
	$buy_group_check =  mysql_num_rows(mysql_query("SHOW COLUMNS FROM ".$config['db']['pre']."buyers"));
	
	if($buy_group_check == 14)
	{
	...
	{
		echo "Buyer table already has ".$buy_group_check." fields<br>";
	}
